...
6. **Evaluation (on demand only, OPTIM-001):** the with-skill vs without-skill
   A/B does not run automatically. Run it when asked, via
   `evals/run_jarvis_eval.py --ticket <id> --agents <csv>` or `/jarvis <id> --eval`.
   It writes `runtime/eval/<id>_<ts>.log` (+ `.json`) and appends
   `runtime/eval-runlog.jsonl`. See `specs/OPTIM-001-token-optimization.md`
   and `specs/EVALS-002-jarvis-inline-eval.md`.
7. Append each step to `runtime/pipeline-runlog.jsonl` (agent, ticket, ts, status).
8. Print a final pipeline summary table (including the evaluation results).
...
- [ ] **Evaluation (EVALS-002 + OPTIM-001):** the with/without-skill A/B is
      available on demand (`/jarvis <id> --eval` or `evals/run_jarvis_eval.py`),
      writing logs under `runtime/eval/`. It does not run automatically.
