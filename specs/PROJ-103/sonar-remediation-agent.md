# Action Spec: Sonar Remediation Agent (Persona: Finops & Devops) - PROJ-103

**Agent:** `sonar-remediation-agent`  ·  Persona: Finops & Devops  ·  Pipeline stage: 4 - Quality/FinOps
...
This document states, for review before execution, exactly what Sonar Remediation Agent (Persona: Finops & Devops)
...
## Agent role
Scans the code for the 132 Sonar rules and fixes the code against those rules.

## Planned action for PROJ-103
Triage did not route Sonar Remediation Agent (Persona: Finops & Devops) for PROJ-103. No action will be taken.

_If it were routed, it would: Scans the code for the 132 Sonar rules and fixes the code against those rules._
