# Action Spec: Governance & Security Agent (Persona: Engineering Leader) - PROJ-103

**Agent:** `governance-security-agent`  ·  Persona: Engineering Leader  ·  Pipeline stage: 5 - Review/Governance
**Ticket:** PROJ-103: Optimize LOAD_FCT_TRADES stored procedure for faster nightly load
**Verdict / severity:** FEATURE / P3   (from Triage)
**Routing status:** NOT ROUTED
**Action status:** NO ACTION
...
This document states, for review before execution, exactly what Governance & Security Agent (Persona: Engineering Leader)
...
## Agent role
Scans the code for security / governance violations and provides a report / fixes the code.

## Planned action for PROJ-103
Triage did not route Governance & Security Agent (Persona: Engineering Leader) for PROJ-103. No action will be taken.

_If it were routed, it would: Scans the code for security / governance violations and provides a report / fixes the code._

## Inputs
n/a (not routed).

## Output artifact(s)
Report only; edits no source.

## Objects in scope (from Triage)
| Object | Type | Path | This ticket's interaction |
|--------|------|------|---------------------------|
_Not routed for PROJ-103; no objects in scope._

## Proposed changes / operations (for review)
| # | File | Operation |
|---|------|-----------|
_None: this agent is not routed for this ticket._
