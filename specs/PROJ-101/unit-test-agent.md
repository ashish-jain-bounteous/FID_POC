# Action Spec: Unit Test Agent — PROJ-101
...
This document states — **for review before execution** — exactly what Unit Test Agent
...
Author unit tests reproducing the defect and proving the fix (cases U-1…U-8 from the master spec): multi-IS_CURRENT fan-out prevention, historical-row exclusion, dedup-guard re-run, NULL-safe guard, UNIQUE enforcement, remediation idempotency & row-selection.

## Inputs
Spec + changed code

## Output artifact(s)
Unit test files/SQL under tests/
...
| # | File | Operation |
|---|------|-----------|
| 1 | tests/PROJ-101/test_load_fct_trades.sql | Create unit-test SQL covering U-1…U-8. |
