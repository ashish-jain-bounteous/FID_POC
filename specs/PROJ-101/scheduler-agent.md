# Action Spec: Scheduler Agent — PROJ-101

**Agent:** `scheduler-agent`  ·  Persona: Data Engineer  ·  Pipeline stage: 2 — Build
...
This document states — **for review before execution** — exactly what Scheduler Agent
...
## Agent role
Create/adjust Control-M (Korous) job definitions and dependencies for the change.

## Planned action for PROJ-101
Triage did not route **Scheduler Agent** for PROJ-101. No action will be taken.

_If it were routed, it would: Create/adjust Control-M (Korous) job definitions and dependencies for the change._

## Inputs
Design spec; control_m/**

## Output artifact(s)
Updated Control-M job JSON
