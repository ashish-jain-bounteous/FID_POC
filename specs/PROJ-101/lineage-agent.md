# Action Spec: Lineage Agent — PROJ-101
...
This document states — **for review before execution** — exactly what Lineage Agent
...
Document end-to-end lineage for the impacted chain:
- `STG_TRADES` → `LOAD_FCT_TRADES` → `FCT_TRADES` → `VW_DAILY_POSITIONS`.
- Dimension feeds: DIM_ACCOUNT, DIM_SECURITY, DIM_DATE.
- Orchestration: Control-M job `CTM_LOAD_FCT_TRADES` (in: STG_TRADES_LOADED-OK; out: FCT_TRADES_LOADED-OK).

## Inputs
Impacted objects from Triage; snowflake/** and control_m/**

## Output artifact(s)
reports/PROJ-101-lineage.md
...
_Read-only analysis — emits `reports/PROJ-101-lineage.md`; edits no source files._
